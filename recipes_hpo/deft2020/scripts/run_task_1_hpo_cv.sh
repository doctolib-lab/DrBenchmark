...
attempt=1
while true; do
  python task_1_finetuning_bert_regr_hpo_cv_v2.py --config="../yaml/regr_hpo.yaml" --model="$MODEL"
  status=$?
  if [ "$status" -eq 0 ]; then
    break
  fi
...
for ((run=1; run<NUM_RUNS; run++)); do
  SEED=$((BASE_SEED + run))
  python task_1_finetuning_bert_regr_hpo_cv_v2.py --config="../yaml/regr_hpo.yaml" --model="$MODEL" --seed="$SEED"
done
