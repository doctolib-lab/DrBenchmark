...
def main():
    ...
    search_pattern = f"../runs/DrBenchmark-DIAMED-cls-{args.subset}-*_hpo.json"
    do_hpo = True
    best_hp_trial = None
    ...
    # Remove duplicates based on title + clinical_case
    seen = set()

    def duplicate_filter(example):
        text = example["title"] + " " + example["clinical_case"]
        if text in seen:
            return False
        seen.add(text)
        return True
    ...
    output_name = f"DrBenchmark-DIAMED-cls-{args.subset}-{str(uuid.uuid4().hex)}"
