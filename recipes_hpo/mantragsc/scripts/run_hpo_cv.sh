...
attempt=1
while true; do
  python finetuning_bert_ner_datamerge_hpo_cv_v2.py --config="../yaml/ner_hpo.yaml" --model="$MODEL" --subset="$SUBSET"
  status=$?
  if [ "$status" -eq 0 ]; then
    break
  fi
...
for ((run=1; run<NUM_RUNS; run++)); do
  SEED=$((BASE_SEED + run))
  python finetuning_bert_ner_datamerge_hpo_cv_v2.py --config="../yaml/ner_hpo.yaml" --model="$MODEL" --subset="$SUBSET" --seed="$SEED"
done
