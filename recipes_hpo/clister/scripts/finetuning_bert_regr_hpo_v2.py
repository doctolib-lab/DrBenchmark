...
def main():
    ...
    search_pattern = f"../runs/DrBenchmark-CLISTER-regression-{args.subset}-*_hpo.json"
    do_hpo = True
    best_hp_trial = None
    ...
    # Remove duplicates based on text_1 + text_2 (bidirectional for similarity tasks)
    seen = set()

    def duplicate_filter(example):
        # Create bidirectional key since order doesn't matter for similarity
        key1 = (example["text_1"], example["text_2"])
        key2 = (example["text_2"], example["text_1"])
    ...
    # Clean test first and use it to deduplicate train/validation against it
    dataset["test"] = dataset["test"].filter(duplicate_filter)
    dataset["validation"] = dataset["validation"].filter(duplicate_filter)
    dataset["train"] = dataset["train"].filter(duplicate_filter)
    ...
    output_name = (
        f"DrBenchmark-CLISTER-regression-{args.subset}-{str(uuid.uuid4().hex)}"
    )
        ...
        json.dump(
            {
                ...
                "predictions": {
                    ...
                    "system_predictions": _predictions.flatten().tolist(),
                },
                "run_seed": args.seed,
            },
            f,
