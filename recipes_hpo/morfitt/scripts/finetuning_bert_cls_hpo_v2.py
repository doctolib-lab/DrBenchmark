...
def main():
    ...
    search_pattern = f"../runs/DrBenchmark-MORFITT-cls-{args.subset}-*_hpo.json"
    do_hpo = True
    best_hp_trial = None
    ...
    # Keep provided splits and existing cleaning/dedup
    labels_list = dataset["train"].features["specialities"].feature.names

    # Remove duplicates (apply to each split independently)
    seen = set()
    ...
    dataset["test"] = dataset["test"].filter(filter_duplicate)
    dataset["validation"] = dataset["validation"].filter(filter_duplicate)
    dataset["train"] = dataset["train"].filter(filter_duplicate)
    ...
    output_name = f"DrBenchmark-MORFITT-cls-{args.subset}-{str(uuid.uuid4().hex)}"
